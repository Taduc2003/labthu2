...
public class Cart {
	...
	public float totalCost() {
		float cost = 0;
		for(int i = 0 ; i < qtyOrdered ; i++) {
			cost = cost + itemOrdered[i].getCost();
		}
		return cost;
	}
    ...
    public void searchByTitle(String title) {
    	boolean found = false; //tạo 1 biến kiểm tra xem có tìm thấy không
    	for(int i = 0; i < qtyOrdered; i++) {
    		if(itemOrdered[i].isMatch(title)) { // Dùng isMatch() để kiểm tra
    			System.out.println("Found DVD with title: " + title);
    			System.out.println(itemOrdered[i].toString());
    			found = true;
    		}
    	}
    	if(found == false) System.out.println("No match found for DVD with title: " + title);
    }
